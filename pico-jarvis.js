...
async function exchange(from, to) {
    ...
    return `As per ${data.time_last_update_utc}, 1 ${from} equal to ${Math.ceil(
        rate
    )} ${from}.`;
}
...
async function ingest(url) {
    ...
    const paginate = (entries, pagination) =>
        entries.map((entry) => {
            const { offset } = entry;
    ...
    const pagination = sequence(pages.length).map((k) =>
        pages
            .slice(0, k + 1)
            .reduce((loc, page) => loc + page.content.length, 0)
    );
...
async function lookup(document, question, hint) {
    ...
    let source = `Best source (page ${top.page + 1}, score ${Math.round(
        top.score * 100
    )}%):\n${top.sentence}`;
...
async function act(document, question, action, observation) {
    ...
    if (name === "lookup") {
        const { result, source, reference } = await lookup(
            document,
            question,
            observation
        );
...
async function reason(document, history, question) {
    ...
    const prompt = REASON_PROMPT.replace(
        "{{CONTEXT}}",
        context(history)
    ).replace("{{QUESTION}}", question);
    ...
    const server = http.createServer(async (request, response) => {
        ...
        if (url === "/health") {
            ...
            if (question === "!reset") {
                ...
                response
                    .writeHead(200)
                    .end("Multi-turn conversation is reset.");
                return;
            }
            ...
            const { thought, action, observation, answer, source, reference } =
                await reason(document, state.history, question);
            const elapsed = Date.now() - start;
            state.source = source;
